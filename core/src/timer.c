...
AddtoRTOList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
	...
	if (cur_stream->on_rto_idx < 0 ) {
		...
		if (diff < RTO_HASH) {
#else
			int offset= ((diff + mtcp->rto_store->rto_now_idx) & (RTO_HASH - 1));
...
RearrangeRTOStore(mtcp_manager_t mtcp) {
	...
	for (walk = TAILQ_FIRST(rto_list);
		...
		if (diff < RTO_HASH) {
			...
			int offset = ((diff + mtcp->rto_store->rto_now_idx) & (RTO_HASH - 1));
			if (!TAILQ_EMPTY(&mtcp->rto_store->rto_list[RTO_HASH])) {
...
CheckRtmTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
	...
	while (1) {
		...
		if (cnt > thresh) {
			...
			mtcp->rto_store->rto_now_idx = ((mtcp->rto_store->rto_now_idx + 1) & (RTO_HASH - 1));
			mtcp->rto_store->rto_now_ts++;
			if (!((mtcp->rto_store->rto_now_idx & (1024 - 1)))) {
...
RegTimer(mtcp_manager_t mtcp, struct timer *timer)
{
	...
	TAILQ_FOREACH_REVERSE(walk, &mtcp->timer_list, timer_head, timer_link) {
		if (TIMEVAL_LT(&walk->exp, &timer->exp)) {
			TAILQ_INSERT_AFTER(&mtcp->timer_list, walk, timer, timer_link);
			return 0;
		}
	}
	...
	TAILQ_INSERT_HEAD(&mtcp->timer_list, timer, timer_link);
	return 0;
}
/*----------------------------------------------------------------------------*/
static struct timer *
NewTimer(mtcp_manager_t mtcp, int id, struct timeval *timeout, callback_t cb)
{
#ifdef USE_TIMER_POOL
	struct timer *t = MPAllocateChunk(mtcp->timer_pool);
#else
...
{
	TAILQ_REMOVE(&mtcp->timer_list, timer, timer_link);
#ifdef USE_TIMER_POOL
	MPFreeChunk(mtcp->timer_pool, timer);
#else
	free(timer);
#endif
}
...
mtcp_settimer(mctx_t mctx, int id, struct timeval *timeout, callback_t cb)
{
	...
	struct timer *t = NewTimer(mtcp, id, timeout, cb);
	if (!t)
		return -1;
