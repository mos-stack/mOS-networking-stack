...
/*----------------------------------------------------------------------------*/
int
psio_get_nif(struct ifreq *ifr)
{
	int i;
	static int num_dev = -1;

	if (num_dev < 0) {
		num_dev = ps_list_devices(devices);
		if (num_dev == -1) {
	...
	for (i = 0; i < num_dev; i++)
		if (!strcmp(ifr->ifr_name, devices[i].name))
			return i;
	return -1;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
int32_t
psio_dev_ioctl(struct mtcp_thread_context *ctx, int nif, int cmd, void *argp)
{
	/* unimplemented */
	return -1;
}
...
io_module_func ps_module_func = {
	...
	.get_nif		   = psio_get_nif,
	.select			   = psio_select,
	.destroy_handle		   = psio_destroy_handle,
	.dev_ioctl		   = psio_dev_ioctl,
};
/*----------------------------------------------------------------------------*/
