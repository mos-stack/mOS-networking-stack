...
/*----------------------------------------------------------------------------*/
rb_manager_t
RBManagerCreate(size_t chunk_size, uint8_t disable_rings, uint32_t concurrency)
{
	...
	rbm->cnum = concurrency;
	rbm->mp = (mem_pool_t)MPCreate(chunk_size, 
	...
	rbm->frag_mp = (mem_pool_t)MPCreate(sizeof(struct fragment_ctx), 
					    sizeof(struct fragment_ctx) * concurrency, 0);
	if (!rbm->frag_mp) {
	...
	rbm->free_fragq = CreateRBFragQueue(concurrency);
	if (!rbm->free_fragq) {
	...
	rbm->free_fragq_int = CreateRBFragQueue(concurrency);
...
RBInit(rb_manager_t rbm, uint32_t init_seq, uint8_t buffer_mgmt)
{
...
/*----------------------------------------------------------------------------*/
void
RBFree(rb_manager_t rbm, struct tcp_ring_buffer* buff, uint8_t buffer_mgmt)
{
	assert(buff);
	if (buff->fctx) {
		...
		uint32_t cur_seq, uint32_t len)
{
	...
	if (GetMinSeq(rcvbuf->head_seq, cur_seq) != rcvbuf->head_seq)
		return 1;
	...
	for (iter = rcvbuf->fctx; iter != NULL; iter = iter->next) {
		if (CanMerge(new_ctx, iter)) {
			FreeFragmentContextSingle(rbm, new_ctx);
			return 1;
		} else if (GetMaxSeq(cur_seq + len, iter->seq) == iter->seq)
...
/*----------------------------------------------------------------------------*/
int
RBPut(rb_manager_t rbm, struct tcp_stream *cur_stream, void *data, 
      uint32_t len, uint32_t cur_seq)
{
	int putx, end_off;
	struct fragment_ctx *new_ctx;
	struct fragment_ctx* iter;
	...
	merged = 0;
	buff = cur_stream->rcvvar->rcvbuf;
	buffer_mgmt = cur_stream->buffer_mgmt;
	...
	if (buff->size <= (buff->head_offset + end_off)) {
		...
		buff->monitor_read_tail_offset = buff->head_offset - 1;
		/* traverse the list and update monitor values */
		SOCKQ_FOREACH_START(walk, &cur_stream->msocks) {
			if (walk->monitor_stream->monitor_read.head_offset[cur_stream->side] >= buff->head_offset) {
				if (walk->monitor_stream->monitor_read.head_offset[cur_stream->side] != buff->head_offset) {
					walk->monitor_stream->monitor_read.overlap[cur_stream->side] = 1;
				}
				walk->monitor_stream->monitor_read.head_offset[cur_stream->side] = 0;
			}
		} SOCKQ_FOREACH_END;
	...
	if (buff->tail_offset < buff->head_offset + end_off) {
		/* traverse the list and update monitor values */
		SOCKQ_FOREACH_START(walk, &cur_stream->msocks) {
			if (buff->tail_offset < walk->monitor_stream->monitor_read.head_offset[cur_stream->side] &&
			    buff->tail_offset + end_off >= 
			    walk->monitor_stream->monitor_read.head_offset[cur_stream->side]) {
				walk->monitor_stream->monitor_read.head_offset[cur_stream->side] = buff->tail_offset + end_off + 1;
				walk->monitor_stream->monitor_read.overlap[cur_stream->side] = 1;
			}
		} SOCKQ_FOREACH_END;
...
/*----------------------------------------------------------------------------*/
size_t
RBRemove(rb_manager_t rbm, struct tcp_ring_buffer* buff, 
