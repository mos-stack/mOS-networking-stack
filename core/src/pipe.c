...
mtcp_pipe(mctx_t mctx, int pipeid[2])
{
	...
	socket[0] = AllocateSocket(mctx, MOS_SOCK_PIPE);
	if (!socket[0]) {
		errno = ENFILE;
		return -1;
	}
	socket[1] = AllocateSocket(mctx, MOS_SOCK_PIPE);
	if (!socket[1]) {
		FreeSocket(mctx, socket[0]->id, MOS_SOCK_PIPE);
		errno = ENFILE;
		return -1;
	}
	...
	if (!pp) {
		/* errno set by calloc() */
		FreeSocket(mctx, socket[0]->id, MOS_SOCK_PIPE);
		FreeSocket(mctx, socket[1]->id, MOS_SOCK_PIPE);
		return -1;
	}
...
PipeRead(mctx_t mctx, int pipeid, char *buf, int len)
{
	...
	if (to_notify) {
		RaiseEventToPair(mtcp, socket, MOS_EPOLLOUT);
	}
	...
	if (pp->buf_len > 0) {
		...
		AddEpollEvent(mtcp->ep, USR_SHADOW_EVENT_QUEUE, socket, MOS_EPOLLIN);
	}
...
PipeWrite(mctx_t mctx, int pipeid, const char *buf, int len)
{
	...
	if (to_notify) {
		RaiseEventToPair(mtcp, socket, MOS_EPOLLIN);
	}
