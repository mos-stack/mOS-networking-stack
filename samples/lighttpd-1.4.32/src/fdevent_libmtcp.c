...
fdevent_libmtcp_epoll_event_del(fdevents *ev, int fde_ndx, int fd)
{
  /*struct mtcp_epoll_event ep;*/
	...
	ep.data.ptr = NULL;
#endif	
	if (0 != mtcp_epoll_ctl(ev->srv->mctx, ev->epoll_fd, MOS_EPOLL_CTL_DEL, fd, NULL/*&ep*/)) {
...
fdevent_libmtcp_epoll_event_set(fdevents *ev, int fde_ndx, int fd, int events)
{
	...
	if (events & FDEVENT_IN)  ep.events |= MOS_EPOLLIN;
	if (events & FDEVENT_OUT) ep.events |= MOS_EPOLLOUT;
	...
	ep.events |= MOS_EPOLLERR | MOS_EPOLLHUP /* | EPOLLET */;

	/*ep.data.ptr = NULL;*/
	ep.data.sock = fd;

	if (0 != mtcp_epoll_ctl(ev->srv->mctx, ev->epoll_fd, add ? MOS_EPOLL_CTL_ADD : MOS_EPOLL_CTL_MOD, fd, &ep)) {
		log_error_write(ev->srv, __FILE__, __LINE__, "SSS",
			"epoll_ctl failed: ", strerror(errno), ", dying");
...
fdevent_libmtcp_epoll_event_get_revent(fdevents *ev, size_t ndx)
{
	...
	if (e & MOS_EPOLLIN) events |= FDEVENT_IN;
	if (e & MOS_EPOLLOUT) events |= FDEVENT_OUT;
	if (e & MOS_EPOLLERR) events |= FDEVENT_ERR;
	if (e & MOS_EPOLLHUP) events |= FDEVENT_HUP;
	if (e & MOS_EPOLLPRI) events |= FDEVENT_PRI;
...
{
	return ev->_epoll_events[ndx].data.sock;
}
/*----------------------------------------------------------------------------*/
static int
